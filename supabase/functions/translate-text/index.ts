...
serve(async (req) => {
  ...
  try {
    ...
    // Skip translation for empty text or if target language is Japanese
    if (!text.trim() || targetLang === 'ja') {
      console.log('Skipping translation - empty text or Japanese target')
      return new Response(
